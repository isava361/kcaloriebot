...
Для этой установки обновление автоматизировано в `scripts/update.sh`. После
получения новой версии скрипта на сервере установи её отдельно от checkout:

```bash
sudo install -m 755 /opt/kcaloriebot/app/scripts/update.sh /usr/local/sbin/kcaloriebot-update
sudo kcaloriebot-update --check
sudo kcaloriebot-update
```

Скрипт сам задаёт пути из нашей установки; переменные `APP_DIR` и другие из
ручного блока ему не нужны. Он явно останавливает оба сервиса, проверяет бэкап,
ставит `.[miniapp]`, выполняет тесты и миграцию одним процессом, затем запускает
бота и web и ждёт успешных проверок, включая HTTP на 18081. Nginx, mtg и
сертификаты при этом не перенастраиваются. `--force` повторяет установку и запуск,
даже если код уже актуален.

При ошибке скрипт сначала останавливает оба процесса. Откат возвращает старый
код и, если миграция уже началась, проверенную копию БД. Состояние БД после
неудачного запуска сохраняется рядом как `failed-before-*.db`; новые записи
из него не переносятся в старую БД автоматически. При неудаче восстановления
службы остаются остановленными. `--no-rollback` также оставляет службы
