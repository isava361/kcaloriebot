    ...
    try:
        ...
        if args == ["disconnect"]:
            ...
            if status.get("status") == "review":
                ...
                text += (
                    "\n\nПеренос остановлен на этом показателе (время в UTC):\n"
                    + _health_sample_text(status["sample"])
                    + "\nСначала остановите команду на iPhone. Затем откройте "
                    "«Здоровье» → нужный показатель → «Показать все данные» "
                    "и посмотрите, есть ли эта запись.\nЕсли запись есть, "
                    "отправьте:\n"
            ...
            for issue in status.get("issues", [])[:3]:
                text += (
                    "\n\nЗапись изменилась после переноса (время в UTC):\nБыло: "
                    + _health_sample_text(issue["previous"])
                    + "\nСтало: "
                    + _health_sample_text(issue["current"])
                    + "\nВручную исправьте или удалите старую запись в «Здоровье», "
                    "затем подтвердите:\n"
